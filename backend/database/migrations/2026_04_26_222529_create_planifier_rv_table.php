...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('planifier_rv', function (Blueprint $table) {
            $table->id();
            $table->foreignId('id_patient')->constrained('patientes', 'id_patient')->onDelete('cascade');
            $table->foreignId('id_personnel')->constrained('personnel_medical', 'id_personnel')->onDelete('cascade');
            $table->unsignedBigInteger('id_rendez_vous')->nullable();
            $table->foreign('id_rendez_vous')
                  ->references('id_rendez_vous')
                  ->on('rendez_vous')
                  ->nullOnDelete();
            $table->string('service', 50)->default('pediatrie');
