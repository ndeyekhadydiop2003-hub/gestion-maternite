...
export default function Compte() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem("user", JSON.stringify({ ...user, login: form.email }));
      setSaved(true);
  ...
  const initiales = (form.nom || "Dr")[0].toUpperCase();
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#d81bb5", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>Mon compte</div>
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "1.8rem", fontWeight: 700, color: "#111827", margin: 0 }}>Préférences et profil</h1>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 20, maxWidth: 900 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #f9e8f7", padding: 28, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
          <div style={{ width: 90, height: 90, borderRadius: 20, background: "linear-gradient(135deg, #d81bb5, #9c27b0)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 32, marginBottom: 16, boxShadow: "0 8px 24px rgba(216,27,181,0.3)" }}>
            {initiales}
          </div>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#111827", marginBottom: 4 }}>{form.nom || "Dr. Diallo"}</div>
          <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 20 }}>Médecin chef de service</div>
          <div style={{ fontSize: 12, color: "#d81bb5", fontWeight: 600, cursor: "pointer" }}>Changer la photo</div>
          ...
          <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 10, marginTop: 20 }}>
            {[
              ...
              { icon: "📍", label: "Localisation", value: "CHRASNT · Thies" },
            ].map((item, i) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #f9e8f7", padding: 28, boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#111827", margin: 0 }}>Informations personnelles</h2>
            </div>

            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>Nom complet</label>
              <input value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })} style={inp} placeholder="Dr. Diallo" />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>Email</label>
              <input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} style={inp} placeholder="diallo@maternite.sn" />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>Téléphone</label>
              <input defaultValue="+221 77 000 00 00" style={inp} placeholder="+221 77 000 00 00" />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={lbl}>Fonction</label>
              <input defaultValue="Médecin chef de service" style={inp} placeholder="Médecin chef de service" />
            </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #f9e8f7", padding: 28, boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#fdf8ff", borderRadius: 12, padding: "14px 18px", border: "1px solid #f0c4eb" }}>
              ...
              <button onClick={() => { setPwModal(true); setPwError(""); setPwSuccess(""); }}
                style={{ background: "linear-gradient(135deg, #d81bb5, #9c27b0)", color: "#fff", border: "none", borderRadius: 20, padding: "8px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                Modifier
              </button>
            </div>
          </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 420, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
              ...
              <button onClick={() => { setPwModal(false); setPw({ ancien: "", nouveau: "", confirmer: "" }); }}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={handleChangePw} disabled={pwLoading}
                style={{ flex: 1, background: pwLoading ? "#e07fcc" : "linear-gradient(135deg, #d81bb5, #9c27b0)", color: "#fff", border: "none", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: pwLoading ? "wait" : "pointer" }}>
                {pwLoading ? "Modification..." : "Confirmer"}
              </button>
              <button onClick={() => { setPwModal(false); setPw({ ancien: "", nouveau: "", confirmer: "" }); }}
                style={{ flex: 1, background: "#fdf8ff", color: "#d81bb5", border: "1px solid #f0c4eb", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                Annuler
              </button>
            </div>
          </div>
