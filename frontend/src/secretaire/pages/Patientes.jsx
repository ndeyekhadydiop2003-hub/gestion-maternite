...
const emptyForm = { nom: "", date_naissance: "", telephone: "", motif: "", groupe_sanguin: "" };
...
export default function Patientes() {
  ...
  const handleAdd = async () => {
    if (!form.nom || !form.date_naissance) { alert("Nom et date de naissance obligatoires."); return; }
    setSaving(true);
    try {
      const res = await fetch(`${API}/patientes`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: `Bearer ${getToken()}` }, body: JSON.stringify(form) });
      const data = await res.json();
  ...
  const handleEdit = async () => {
    setSaving(true);
    try {
      const res = await fetch(`${API}/patientes/${viewModal.id_patient}`, { method: "PUT", headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: `Bearer ${getToken()}` }, body: JSON.stringify(editForm) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Erreur");
  ...
  const openView = (p) => {
    setViewModal(p);
    setEditForm({ nom: p.nom, date_naissance: p.date_naissance, telephone: p.telephone || "", motif: p.motif || "", groupe_sanguin: p.groupe_sanguin || "" });
    setEditMode(false);
  };

  const filtered = patientes.filter(p => {
    const matchNom    = !search || p.nom?.toLowerCase().includes(search.toLowerCase()) || String(p.id_patient).includes(search);
  ...
  return (
    <div style={{ fontFamily: "'Nunito', sans-serif" }}>
      ...
      {/* ── Filtres avancés ── */}
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f9e8f7", padding: "18px 20px", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14 }}>Filtres de recherche</div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", display: "block", marginBottom: 5 }}>Date inscription</label>
            <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)}
              style={{ padding: "8px 12px", border: "1px solid #f0c4eb", borderRadius: 8, fontSize: 13, background: "#fdf8ff", outline: "none", color: "#111827" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", display: "block", marginBottom: 5 }}>Date naissance</label>
            <input type="date" value={filterDateNaissance} onChange={e => setFilterDateNaissance(e.target.value)}
              style={{ padding: "8px 12px", border: "1px solid #f0c4eb", borderRadius: 8, fontSize: 13, background: "#fdf8ff", outline: "none", color: "#111827" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", display: "block", marginBottom: 5 }}>Groupe sanguin</label>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", display: "block", marginBottom: 5 }}>Téléphone</label>
            <input placeholder="Ex: 77 000" value={filterTel} onChange={e => setFilterTel(e.target.value)}
              style={{ padding: "8px 12px", border: "1px solid #f0c4eb", borderRadius: 8, fontSize: 13, background: "#fdf8ff", outline: "none", color: "#111827", width: 130 }} />
          </div>
          {(filterDate || filterDateNaissance || filterGroupe || filterTel) && (
            <button onClick={() => { setFilterDate(""); setFilterDateNaissance(""); setFilterGroupe(""); setFilterTel(""); }}
              style={{ padding: "8px 16px", background: "#fdf0fb", color: "#d81bb5", border: "1px solid #f5d0f0", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Réinitialiser
            </button>
          )}
      ...
      {/* Tableau — sans colonnes Motif et État */}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #fce7f3" }}>
                {["DÉBUT", "NOM", "DATE NAISSANCE", "TÉLÉPHONE", "GROUPE SANGUIN", "ACTIONS"].map(h => (
            ...
            <tbody>
              {filtered.map((p, idx) => {
                ...
                return (
                  <tr key={p.id_patient} style={{ borderBottom: "1px solid #fdf8ff", transition: "background 0.1s", cursor: "default" }}
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 20px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <Avatar nom={p.nom} idx={idx} />
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#111827" }}>{p.nom}</span>
                      </div>
                    </td>
      ...
      {viewModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 460, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
              <Avatar nom={viewModal.nom} idx={0} />
              <div style={{ flex: 1 }}>
                <h2 style={{ fontSize: 17, fontWeight: 800, color: "#111827", margin: 0 }}>{viewModal.nom}</h2>
              ...
              <>
                {[
                  ...
                  <div key={f.key} style={{ marginBottom: 14 }}>
                    ...
                    <input type={f.type} value={editForm[f.key] || ""} onChange={e => setEditForm({ ...editForm, [f.key]: e.target.value })} style={inp} />
                  </div>
                ))}
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={handleEdit} disabled={saving} style={{ flex: 1, background: "linear-gradient(135deg, #d81bb5, #9c27b0)", color: "#fff", border: "none", borderRadius: 24, padding: "11px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 480, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input placeholder="Ex: Aminata Diallo" value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })} style={inp} />
            </div>
            <div style={{ marginBottom: 14 }}>
