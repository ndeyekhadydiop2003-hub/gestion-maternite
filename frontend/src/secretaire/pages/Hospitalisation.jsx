...
export default function Hospitalisation() {
  ...
  // ✅ CORRECTION 1 : Filtrer seulement les lits libres/disponibles
  const fetchLits = async () => {
    try {
      const res = await fetch(`${API}/lits`, { headers: { Accept: "application/json", Authorization: `Bearer ${getToken()}` } });
      const data = await res.json();
      const tousLits = Array.isArray(data) ? data : data.data || [];
      setLits(tousLits.filter(l => l.statut === "libre" || l.statut === "disponible"));
    } catch (_) {}
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setNotification({ msg: "Admission enregistrée avec succès", type: "success" });
      setTimeout(() => setNotification(null), 5000);
      fetchAdmissions(); fetchLits();
    } catch (err) { alert("Erreur : " + err.message); } finally { setSaving(false); }
  };

  // ✅ CORRECTION 3 : Date sortie obligatoire pour statut terminée
  const handleSortie = async () => {
    if (!dateSortie) { setSortieError("La date de sortie est obligatoire pour terminer une hospitalisation."); return; }
    setSortieLoading(true); setSortieError("");
    try {
      const res = await fetch(`${API}/hospitalisations/${sortieModal.id_hospitalisation}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: `Bearer ${getToken()}` },
        body: JSON.stringify({ date_sorti: dateSortie, statut: "terminee" })
      });
  ...
  const handleEdit = async () => {
    // Bloquer si statut terminee sans date_sorti
    if (editForm.statut === "terminee" && !editForm.date_sorti) {
      alert("La date de sortie est obligatoire pour terminer une hospitalisation.");
      return;
    }
    setSaving(true);
    try {
  ...
  const openView = (a) => { setViewModal(a); setEditForm({ motif: a.motif, statut: a.statut, date_sorti: a.date_sorti || "" }); setEditMode(false); };
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 28 }}>
        {[
          { v: lits.length, l: "Lits disponibles", sub: "En maternité", border: "#bbf7d0", icon: <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#dcfce7", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#15803d" strokeWidth="1.8"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div> },
          { v: actives, l: "Hospitalisations actives", sub: "En cours", border: "#ddd6fe", icon: <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#fce7f3", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d81bb5" strokeWidth="1.8"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></div> },
          { v: enAttente, l: "En attente", sub: "Orientation en cours", border: "#fed7aa", icon: <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#e0f2fe", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0284c7" strokeWidth="1.8"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div> },
          { v: terminees, l: "Sorties", sub: "Enregistrées", border: "#bae6fd", icon: <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#fef9c3", display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ca8a04" strokeWidth="1.8"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div> },
        ].map((s, i) => (
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 500, color: "#6b7280" }}>{s.l}</span>
              {s.icon}
            </div>
            <div style={{ fontSize: 40, fontWeight: 700, color: "#111827", lineHeight: 1, marginBottom: 6, fontFamily: "'Playfair Display', Georgia, serif" }}>{s.v}</div>
            ...
            {editMode ? (
              <>
                <div style={{ marginBottom: 14 }}>
                  ...
                  {/* ✅ CORRECTION 2 : Motif en liste déroulante dans le modal modifier aussi */}
                  <select value={editForm.motif || ""} onChange={e => setEditForm({ ...editForm, motif: e.target.value })} style={inp}>
                ...
                {/* ✅ CORRECTION 3 : Afficher champ date sortie si statut terminee */}
                {editForm.statut === "terminee" && (
                  <div style={{ marginBottom: 14 }}>
                    <label style={lbl}>Date de sortie *</label>
                    <input type="date" value={editForm.date_sorti || ""} onChange={e => setEditForm({ ...editForm, date_sorti: e.target.value })}
                      style={{ ...inp, border: "1px solid #bbf7d0", background: "#f0fdf4" }} />
                    <p style={{ fontSize: 11, color: "#be185d", marginTop: 4 }}>Obligatoire pour terminer une hospitalisation.</p>
                  </div>
                )}
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 480, maxWidth: "90vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>{lits.length === 0 ? "⚠ Aucun lit disponible" : `${lits.length} lit(s) disponible(s)`}</p>
              </div>
            ...
            {priorite && form.motif.length > 2 && (
              <div style={{ background: priorite.bg, color: priorite.color, borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 13, fontWeight: 600 }}>
                Priorité : <strong>{priorite.label}</strong>
              </div>
            )}
            <div style={{ marginBottom: 14 }}>
            ...
            {/* ✅ CORRECTION 2 : Motif en liste déroulante */}
            <div style={{ marginBottom: 14 }}>
              <label style={lbl}>Motif *</label>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={form.id_lit} onChange={e => setForm({ ...form, id_lit: e.target.value })} disabled={lits.length === 0}
                style={{ ...inp, background: lits.length === 0 ? "#f9fafb" : "#fdf8ff", cursor: lits.length === 0 ? "not-allowed" : "pointer" }}>
                <option value="">{lits.length === 0 ? "Aucun lit disponible" : "-- Sélectionner un lit --"}</option>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleSubmit} disabled={saving}
                style={{ flex: 1, background: saving ? "#e07fcc" : "linear-gradient(135deg, #d81bb5, #9c27b0)", color: "#fff", border: "none", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: saving ? "wait" : "pointer" }}>
                {saving ? "Enregistrement..." : "Admettre en Maternité"}
              </button>
              <button onClick={() => { setShowModal(false); setForm(emptyForm); }}
                style={{ flex: 1, background: "#fdf8ff", color: "#d81bb5", border: "1px solid #f0c4eb", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Annuler</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 400, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{sortieModal.patiente?.nom} · {sortieModal.lit ? `${sortieModal.lit.numero_lit}` : ""}</p>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input type="date" value={dateSortie} onChange={e => setDateSortie(e.target.value)}
                style={{ ...inp, border: "1px solid #bbf7d0", background: "#f0fdf4" }} />
              <p style={{ fontSize: 11, color: "#be185d", marginTop: 6, fontWeight: 600 }}>⚠ Obligatoire — le lit sera libéré automatiquement.</p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleSortie} disabled={sortieLoading}
                style={{ flex: 1, background: sortieLoading ? "#86efac" : "linear-gradient(135deg, #15803d, #14532d)", color: "#fff", border: "none", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: sortieLoading ? "wait" : "pointer" }}>
                {sortieLoading ? "Enregistrement..." : "Confirmer la sortie"}
              </button>
              <button onClick={() => { setSortieModal(null); setDateSortie(""); setSortieError(""); }}
                style={{ flex: 1, background: "#fdf8ff", color: "#d81bb5", border: "1px solid #f0c4eb", borderRadius: 24, padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Annuler</button>
            </div>
          </div>
        </div>
