...
const getNewbornId = (p) => {
  const id1 = p.consultation?.pediatrie?.id_nouveau_ne;
  if (id1) return id1;
  const id2 = p.patiente?.nouveau_nes?.[0]?.id_nouveau_ne;
  if (id2) return id2;
  return null;
};
            ...
            <table className="dtable">
              ...
              <tbody>
                {prescriptions.map((p) => {
                  ...
                  const nouveauNeId = getNewbornId(p);
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button style={btnDetail}
                            onClick={() => nouveauNeId
                              ? onNavigate('dossier-detail', nouveauNeId)
                              : onNavigate('dossiers')
                            }>
                            👁️ Détail
                          </button>
                          <button style={btnEdit}
                            onClick={() => onNavigate('ajouter-traitement', nouveauNeId, null, p.id_prescription)}>
                            ✏️
                          </button>
                        </div>
                      </td>
            ...
            <table className="dtable">
              ...
              <tbody>
                {vaccins.map((v) => {
                  ...
                  // ── Nom assigné ──
                  const assigne    = v.assigne ?? null;
                  const nomAssigne = assigne?.utilisateur
                    ? `${assigne.utilisateur.prenom} ${assigne.utilisateur.nom} — ${
                        assigne.utilisateur.role_acces === 'sage_femme' ? 'Sage-femme' : 'Infirmier(e)'
                      }`
                    : v.id_assigne ? `Personnel #${v.id_assigne}` : '—';
                  ...
                  return (
                    <tr key={v.id_vaccin}>
                      ...
                      <td style={{ fontWeight: 600 }}>{v.nom_vaccin?.replace(/_/g, ' ') ?? '—'}</td>
                      <td>{formatDate(v.date_administration)}</td>
                      <td><span className={`badge ${badge.cls}`}>{badge.label}</span></td>
                      <td style={{ fontSize: 12 }}>{nomAssigne}</td>
                      <td style={{ fontSize: 12, maxWidth: 160 }}>{v.observations ?? '—'}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button style={btnDetail}
                            onClick={() => onNavigate('dossier-detail', v.id_nouveau_ne ?? null)}>
                            👁️ Détail
                          </button>
                          {/* ── Pédiatre peut modifier observations + assignation ── */}
                          <button style={btnEdit}
                            // Passer idVaccin en 5ème position
                            onClick={() => onNavigate('ajouter-vaccin', v.id_nouveau_ne ?? null, null, null, v.id_vaccin)}>
                            ✏️
                          </button>
                        </div>
                      </td>
            ...
            <table className="dtable">
              ...
              <tbody>
                {soins.map((s) => {
                  ...
                  const patiente = nn?.accouchement?.grossesse?.patiente ?? nn?.patiente ?? s.patiente ?? null;
                  const nomBebe  = patiente
                  ...
                  const bebeId      = nn?.id_nouveau_ne ?? s.id_nouveau_ne ?? null;

                  // ── Nom assigné (infirmier/sage-femme) ──
                  const assigne    = s.assigne ?? null;
                  const nomAssigne = assigne?.utilisateur
                    ? `${assigne.utilisateur.prenom} ${assigne.utilisateur.nom} — ${
                        assigne.utilisateur.role_acces === 'sage_femme' ? 'Sage-femme' : 'Infirmier(e)'
                      }`
                    : s.id_assigne ? `Personnel #${s.id_assigne}` : '—';
                  ...
                  return (
                    <tr key={s.id_soin}>
                      ...
                      <td style={{ fontSize: 12, maxWidth: 140, color: 'var(--gray)' }}>{s.note ?? '—'}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button style={btnDetail}
                            onClick={() => onNavigate('dossier-detail', bebeId)}>
                            👁️ Détail
                          </button>
                          <button style={btnEdit}
                            onClick={() => onNavigate('planifier-soin', bebeId, s.id_soin)}>
                            ✏️
                          </button>
                        </div>
                      </td>
