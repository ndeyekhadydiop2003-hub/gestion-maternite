...
// Dans ton fichier React, remplace la logique de handleSubmit :
const handleSubmit = async () => {
  try {
    ...
    const payload = {
      id_nouveau_ne:     id ?? Number(form.id_nouveau_ne),
      id_patient:        bebe?.accouchement?.grossesse?.patiente?.id_patient 
                         ?? bebe?.id_patient 
                         ?? null,

      // === Champs de la table consultations ===
      date_consultation:   form.date_consultation,
      motif_consultation:  form.motif_consultation?.trim() || 'Suivi pédiatrique',   // ← CORRECTION
      poids:               form.poids ? Number(form.poids) : null,
      temperature:         form.temperature ? Number(form.temperature) : null,
      
      // Tension en format texte (ex: "120/80")
      tension: form.tension_systolique && form.tension_diastolique 
               ? `${form.tension_systolique}/${form.tension_diastolique}` 
               : null,

      observation:   form.observation?.trim() || null,
      
      // === Champs de la table consultation_pediatrie ===
      ...
      developpement:     form.developpement?.trim() || null,
      vaccin_a_jour:     form.vaccin_a_jour ? true : false,
      vaccins_notes:     form.vaccins_notes?.trim() || null,

      id_personnel: getIdPersonnel(),   // Important
    };

    const response = await api.post('/consultations-pediatrie', payload);

    setSuccess(true);

    setTimeout(() => {
      onNavigate(id ? 'dossier-detail' : 'dossiers', id ?? payload.id_nouveau_ne);
    }, 1500);
