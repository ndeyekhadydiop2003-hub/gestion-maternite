...
export default function PlanifierSoin({ onNavigate, id, idSoin }) {
  const [bebe,        setBebe]        = useState(null);
  const [babies,      setBabies]      = useState([]);
  const [personnel,   setPersonnel]   = useState([]);
  const [loading,     setLoading]     = useState(false);
  const [loadingInit, setLoadingInit] = useState(!!idSoin);
  ...
  // ── Mode édition : charger le soin existant ──
  useEffect(() => {
    if (!idSoin) return;
    api.get(`/soins/${idSoin}`)
      .then(res => {
        const s = res.data;
        setForm({
          id_nouveau_ne: String(s.id_nouveau_ne ?? id ?? ''),
          type_soin:     s.type_soin  ?? '',
          date_soin:     s.date_soin?.slice(0, 10) ?? '',
          heure_soin:    s.heure_soin ?? '',
          note:          s.note       ?? '',
          frequence:     s.frequence  ?? 'unique',
          id_assigne:    s.id_assigne ? String(s.id_assigne) : '',
        });
      })
      .catch(() => setError('Impossible de charger le soin.'))
  ...
  // ── Charger le bébé quand sélectionné ──
  useEffect(() => {
  ...
  console.log('id_assigne sélectionné:', form.id_assigne);
    try {
      setLoading(true);
      setError(null);
      ...
      if (isModification) {
        await api.put(`/soins/${idSoin}`, payload);
      } else {
        await api.post('/soins', payload);
      }
    ...
    <div className="page-anim">
      ...
      <div className="page-title">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <h2>{isModification ? '✏️ Modifier le soin' : '🩺 Planifier un soin'}</h2>
        </div>
      </div>
      ...
      {isModification && (
        <div style={{
          ...
          ✏️ Modification — <strong style={{ marginLeft: 4 }}>{form.type_soin}</strong>
        </div>
      )}
      ...
      <div className="card" style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            ...
            <select name="type_soin" value={form.type_soin}
              onChange={handleChange} style={inputStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              ...
              <input type="date" name="date_soin" value={form.date_soin}
                onChange={handleChange} style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Heure</label>
              <input type="time" name="heure_soin" value={form.heure_soin}
                onChange={handleChange} style={inputStyle} />
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <select name="frequence" value={form.frequence}
              onChange={handleChange} style={inputStyle}>
          ...
          {success && (
            <div style={{
              ...
              <span>✅</span>
              Soin {isModification ? 'modifié' : 'planifié'} avec succès ! Redirection...
            </div>
          )}
          ...
          <div style={{
            ...
            <button className="btn-primary" onClick={handleSubmit}
              disabled={loading || !form.type_soin || !form.date_soin || (!id && !form.id_nouveau_ne)}
              style={{ minWidth: 160 }}>
              {loading
                ? '⏳ Enregistrement...'
                : isModification
