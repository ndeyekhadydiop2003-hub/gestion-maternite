...
// ── Nom du personnel depuis personnel_medical directement ──
const getNomPersonnel = (personnel) => {
  if (!personnel) return null;
  // Priorité : nom/prenom du personnel_medical
  if (personnel.nom && personnel.prenom) return `${personnel.prenom} ${personnel.nom}`;
  if (personnel.prenom) return personnel.prenom;
  if (personnel.nom) return personnel.nom;
  // Fallback : utilisateur
  if (personnel.utilisateur?.prenom || personnel.utilisateur?.nom)
    return `${personnel.utilisateur.prenom || ''} ${personnel.utilisateur.nom || ''}`.trim();
  // Fallback final : login
  if (personnel.utilisateur?.login) return personnel.utilisateur.login;
  return personnel.fonction || null;
};
...
export default function RightPanel({ babyId = null, onNavigate = () => {} }) {
  ...
  const patiente   = bebe?.accouchement?.grossesse?.patiente ?? bebe?.patiente ?? null;
  const patienteId = patiente?.id_patient ?? bebe?.id_patient ?? null;
...
// ══════════════════════════════════════════════════════════
// Consultations filtrées par babyId — TOUTES affichées avec scroll
// ══════════════════════════════════════════════════════════
// ConsultationsBebe corrigée (basée sur le fichier qui fonctionne)
function ConsultationsBebe({ babyId = null, onNavigate = () => {} }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get(`/nouveau-nes/${babyId}/consultations-pediatrie`)
      .then(res => {
        const all = Array.isArray(res.data) ? res.data : [];
        const cons = all.map(p => ({
          id_cp:              p.id_cp,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          ...
          <button onClick={() => setSelectedConsultation(null)} style={{
        ...
        {[
          ['📅 Date', new Date(c.date_consultation).toLocaleDateString('fr-FR')],
          ['📝 Motif', c.motif_consultation || '—'],
          ['⚖️ Poids', c.poids ? `${c.poids} kg` : '—'],
          ['🌡️ Température', c.temperature ? `${c.temperature} °C` : '—'],
          ['💓 Tension', c.tension || '—'],
          ['📏 Taille', c.taille ? `${c.taille} cm` : '—'],
          ['🧠 Périmètre crânien', c.perimetre_cranien ? `${c.perimetre_cranien} cm` : '—'],
          ['🤱 Allaitement', c.allaitement || '—'],
          ['🌱 Développement', c.developpement || '—'],
          ['💉 Vaccins à jour', c.vaccin_a_jour ? '✅ Oui' : '❌ Non'],
        ].map(([label, value]) => (
          <div key={label} style={{
        ...
        {c.observation && (
          <div style={{ marginTop: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--rose-2)', textTransform: 'uppercase', marginBottom: 4 }}>
              🔬 Observation clinique
            </div>
            <div style={{ fontSize: 12, color: 'var(--dark)', background: '#f9f9fc', borderRadius: 8, padding: '8px 10px', border: '1px solid var(--sand)' }}>
              {c.observation}
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
          <button
            ...
            onClick={() => {
              onNavigate('dossier-detail', babyId);
              setSelectedConsultation(null);
            }}
          >
            ✏️ Modifier cette consultation
          </button>
          <button
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <button className="btn-primary btn-sm"
        style={{ width: '100%', marginBottom: 8, background: 'var(--teal)' }}
          ...
          {consultations.map(c => (
            <div key={c.id_consultation} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {c.poids && <span className="badge b-gray" style={{ fontSize: 10 }}>{c.poids} kg</span>}
              </div>
...
function PrescriptionsBebe({ patienteId = null, onNavigate = () => {}, babyId = null }) {
  ...
  useEffect(() => {
    ...
    api.get(`/prescriptions?id_patient=${patienteId}&per_page=20`)
      .then(res => {
        const all = res.data?.data ?? res.data ?? [];
        setPrescriptions(Array.isArray(all) ? all : []);
      })
