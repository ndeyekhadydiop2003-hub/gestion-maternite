...
import { getRedirectByRole } from '../services/authService';
...
const LoginPage = () => {
  ...
  const [stats, setStats] = useState({ naissances: '0', satisfaction: '98%', service: '24h/7' });

  useEffect(() => {
    setLogin(''); setPassword(''); setError('');
    setTimeout(() => { setLogin(''); setPassword(''); }, 50);
  }, []);

  useEffect(() => {
    const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';
    fetch(`${apiUrl}/stats/public`, { headers: { Accept: 'application/json' } })
      .then(r => { if (!r.ok) throw new Error(); return r.json(); })
      .then(data => setStats({
        naissances: data.total_naissances !== undefined
          ? Number(data.total_naissances).toLocaleString('fr-FR') + (data.total_naissances > 0 ? '+' : '') : '0',
        satisfaction: data.satisfaction ? data.satisfaction + '%' : '98%',
        service: '24h/7',
      }))
      .catch(() => setStats({ naissances: '0', satisfaction: '98%', service: '24h/7' }));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // getRedirectByRole gère TOUS les rôles : secretaire, medecin_chef, admin, etc.
      const route = getRedirectByRole(data.utilisateur?.role_acces);
    ...
    <div style={{ fontFamily:"'Nunito', sans-serif", background:'#FFF0F5', height:'100vh', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .brand-text h1 { font-family:'Cormorant Garamond',serif; font-size:15px; font-weight:700; color:#fff; line-height:1.3; }
        ...
        .hero-title { font-family:'Cormorant Garamond',serif; font-size:34px; line-height:1.1; color:#fff; font-weight:600; margin-bottom:1rem; }
        .hero-title em { font-style:italic; color:#F8BBD9; }
        .hero-sub { font-size:14px; line-height:1.6; color:rgba(255,255,255,0.95); max-width:360px; }
        .stats { position:relative; z-index:3; display:grid; grid-template-columns:repeat(3,1fr); gap:8px; }
        ...
        .right { flex:0.9; background:#FFF8FB; display:flex; flex-direction:column; justify-content:center; padding:1rem 2.5rem 1rem; position:relative; }
        .top-badge { position:absolute; top:1.5rem; right:1.5rem; display:flex; align-items:center; gap:12px; }
        .top-mini-logo { width:44px; height:44px; background:#C2185B; border-radius:14px; display:flex; align-items:center; justify-content:center; }
        ...
        .inp-icon { position:absolute; left:16px; top:50%; transform:translateY(-50%); width:18px; height:18px; display:flex; align-items:center; justify-content:center; font-size:15px; pointer-events:none; }
        .inp-field { width:100%; padding:12px 18px 12px 46px; border:2px solid #F8BBD9; border-radius:18px; font-family:'Nunito',sans-serif; font-size:15px; background:#FFF0F5; color:#880E4F; outline:none; transition:all 0.3s ease; }
        ...
        .btn-login { width:100%; padding:13px; background:#C2185B; color:#fff; border:none; border-radius:20px; font-family:'Nunito',sans-serif; font-size:15px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:12px; transition:all 0.3s ease; margin-bottom:0.8rem; box-shadow:0 6px 20px rgba(194,24,91,0.25); }
        .btn-login:hover:not(:disabled) { background:#880E4F; transform:translateY(-2px); }
        ...
        @media (max-width:1000px) { .left { display:none; } .root { max-width:600px; } }
      `}</style>
        ...
        <div className="left">
          ...
          <div className="brand">
            ...
            <div className="brand-text">
              <h1>Maternité Poste de Santé CHRASNT</h1>
          ...
          <div className="stats">
            {[{n:stats.naissances,l:'Naissances'},{n:stats.satisfaction,l:'Satisfaction'},{n:stats.service,l:'Service'}].map((s,i)=>(
        ...
        <div className="right">
          <div className="top-badge">
            ...
            <div className="top-mini-name">
              <div className="a">Maternité CHRASNT</div>
          ...
          <form onSubmit={handleSubmit} autoComplete="off">
            ...
            <div className="inp-wrap">
              ...
              <input type="text" value={login} autoComplete="new-password" onChange={e => setLogin(e.target.value)} className="inp-field" placeholder="Votre identifiant" required />
            </div>
            <label className="flabel">Mot de passe</label>
            <div className="inp-wrap">
              <div className="inp-icon">🔒</div>
              <input type="password" value={password} autoComplete="new-password" onChange={e => setPassword(e.target.value)} className="inp-field" placeholder="Votre mot de passe" required />
            </div>
            ...
            <button type="submit" className="btn-login" disabled={loading}>
              {loading ? <><div className="spinner"/>Connexion en cours…</> : 'Se connecter →'}
            </button>
          </form>
